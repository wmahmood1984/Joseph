    ...
    async(a,thunkApi)=>{
        ...
        try {
            if(Web3.givenProvider){ 
				...
				const SeekGoldAddress = "0x1D23e9259E1B6eb8d9b5D4aF790Bc45b5A0e7C4D"
				var contract = new web3.eth.Contract(abi, SeekGoldAddress);
                MultiSendAddress = SeekGoldAddress;
...
export const MultiSend = createAsyncThunk("MultiSend",
    async ({selectedToken,contributors,balances})=>{
	console.log("SelectedToken",selectedToken)
	console.log("Contributors",contributors)
	console.log("Balances",balances)
	
	var localBalance = balances.map(transaction => web3.utils.toWei(transaction.toString(),"ether"))	
	var tokenContract = new web3.eth.Contract(abi, MultiSendAddress);
	try {
		const result = await tokenContract.methods.multisendToken
		(selectedToken, contributors, localBalance)
...
export const Approval = createAsyncThunk("Approval",
async ({token, deflationary ,tokenValue})=>{
	
	var localAbi = deflationary? deflationaryAbi : tokenAbi;
	var tokenContract =new web3.eth.Contract(localAbi, token);
	console.log("multi",token)
	try {
		const result = await tokenContract.methods.approve
		(MultiSendAddress, web3.utils.toWei(tokenValue.toString(),"ether"))
		.send({from : address,  gas: 3000000})
		return result;
	} catch (error) {
		console.log("Error in Approve Function",error)
...
const adoptSlice = createSlice({
    ...
    extraReducers: {
		...
		[MultiSend.pending] : (state,action)=>{
			state.arrayAwait = true;
            state.toggle = !state.toggle;
			state.error = null;
        },
        [MultiSend.fulfilled] : (state,action)=>{
